...
- 🛡️ **防封客户端仿真（新增）**：网关支持 `clientUA` 配置——完全仿真 Claude Code 客户端访问特征，通过 new-api/one-api 的"unauthorized client detected"客户端白名单检测；仿真模式下**不透传任何 dsh/网关特征头**，上游无法识别代理
- 🧯 **请求风暴防护**：catalog 探测失败 30 秒冷却（不再每次请求都重试上游），防止故障时高频请求触发风控
- 🔌 **baseURL 智能补全**：自动补 `/v1` 后缀（兼容两种填写习惯）、收敛误填的完整路径
