...
- 🧩 **界面升级**：主界面改为 **Tab 双页**（「dsh 服务」/「模型网关」）；供应商改为**可视化表格编辑**（DataGridView：增删改查 + 勾选启用 + 一键保存），不再裸开配置文件
- 💿 **安装版（新增）**：单文件安装器 `DSHDesktopSetup.exe` —— 图形向导 / 静默安装、自动创建桌面与开始菜单快捷方式、写入卸载注册表，卸载时保留 `data\` 用户数据
- 🛡️ **防屏蔽透传**：网关转发时保持 dsh 原始请求标识（`User-Agent: deepseek-harness/...`），上游视为 dsh 直连；SSE 流式强制 `identity` 编码（修复流式压缩乱码）
- 🚪 **模型网关（new-api 风格）**：多供应商统一代理、可用性探测、优先级路由、故障切换、SSE 流式透传、一键写入 dsh 配置
- 📂 **绿色便携**：数据目录随 exe（`data\`），整体拷贝即可迁移；支持 `DSH_DATA_DIR` 指定位置
- ✅ 加固：YAML/JSON 注入防护、请求体上限、并发去重、日志轮转、崩溃兜底、无限审计修复；内置一键回归测试（`run-all-tests.ps1`，网关 E2E 11 项）
