...
- 🔀 **路由模式双选**：同一模型多供应商支持「主备(优先级)」与「轮询(流量分摊)」两种模式——轮询按请求轮流切换首选供应商（保留失败切换），适合额度分摊/负载均衡
- 🛡️ **防封客户端仿真**：网关支持 `clientUA` 配置——完全仿真 Claude Code 客户端访问特征，通过 new-api/one-api 客户端白名单检测；仿真模式**不透传任何 dsh/网关特征头**
- 🧯 **请求风暴防护**：catalog 探测失败 30 秒冷却，防止故障时高频请求触发风控
- 📜 **日志体系升级**：网关日志与 dsh 日志**分离展示**；新增**调用日志**（每次请求：模型→路由→状态→耗时→来源）；两页日志均带**语义着色**（错误红/成功绿/调用青/警告黄）+ 自动换行 + 清屏快进
- 🎨 **按钮交互增强**：全部按钮 hover/按下反馈 + 手型光标 + **运行态自动联动**（运行中禁用"启动"、启用"停止"，状态色随动）
- 🔌 **baseURL 智能补全**：自动补 `/v1` 后缀、收敛误填的完整路径
