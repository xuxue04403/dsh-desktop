...
- **壳级入口常驻**：dsh 页面右上角悬浮「⚙ 设置 / 🛡 模型网关」按钮——**固定定位在页面右上角工具按钮（Session 下载等）的左侧同行**（注入后探测 3 次锚点 + resize 重定位；锚点必须完全在视口内，左侧空间不足时自动落到右下角，绝不遮挡页面按钮，也不会跳动）；服务就绪后仍可直达并定位高亮；托盘菜单同款入口；
